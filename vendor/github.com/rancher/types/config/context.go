...
import (
	...
	huaweiv3 "github.com/rancher/types/apis/cloud.huawei.com/v3"
	huaweiSchema "github.com/rancher/types/apis/cloud.huawei.com/v3/schema"
...
func NewScaledContext(config rest.Config) (*ScaledContext, error) {
	...
	context.HuaWei, err = huaweiv3.NewForConfig(config)
	if err != nil {
		return nil, err
	}
	...
	context.Schemas = types.NewSchemas().
		AddSchemas(huaweiSchema.Schemas).
...
func NewManagementContext(config rest.Config) (*ManagementContext, error) {
	...
	corev1.AddToScheme(context.Scheme)
	huaweiv3.AddToScheme(context.Scheme)
